function censorship(text, censoredStrings) {
    ...
    for (let str of censoredStrings) {
        let startIndex = censoredText.indexOf(str);
        while (startIndex>-1) {
            censoredText = censoredText.replace(str,"-".repeat(str.length));
            startIndex = censoredText.indexOf(str);
        }
    }
