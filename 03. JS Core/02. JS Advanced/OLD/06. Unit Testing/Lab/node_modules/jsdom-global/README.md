...
* Running it with [smokestack]:
  ...
  ```sh
  browserify pesho.js | smokestack          # run in a browser
  node pesho.js                             # or the console
  browserify pesho.js --no-bundle-external  # also works (but why bother?)
  ```

* Running it with Babel ([babelify] or [babel-cli]):

  ```sh
  browserify pesho.js -t babelify | smokestack  # run in a browser (with babel)
  babel-node pesho.js                           # or the console
  ```
